...
Sans les données, l'application ne peut pas être lancée localement. Le dossier images_app/ contient quelques captures d'écran illustrant partiellement l'interface et certaines fonctionnalités de l'application. Ces aperçus sont basés sur un exemple d'une seule base de données pour les parties Présentation et Analyse, ainsi que sur la zone Bananord uniquement pour la partie Prédiction & Alertes. Seuls quelques indicateurs sont affichés à titre d'exemple et ne reflètent pas l'ensemble des fonctionnalités disponibles dans l'application complète.
...
1. Cloner le repository :
   ```bash
   git clone https://github.com/githk-1/Projet_cercos_dashboard.git
   cd projet_cerco
   ```
